...
interface IForm {
  name: string;
  price: string; // every type on form is 'string'
  description: string;
  [key: string]: string;
}
...
export const AddDish = () => {
  ...
  const { register, handleSubmit, formState, getValues, setValue } = useForm<IForm>({
    mode: "onChange", // validates when every input changes
  });
  ...
  const onSubmit = () => {
    ...
    createDishMutation({
      variables: {
        input: {
          name,
          price: +price, // convert to number type via '+'
          description,
    ...
    history.goBack(); // shows previous page
  };
